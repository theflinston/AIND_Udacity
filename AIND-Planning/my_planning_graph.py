...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if len(set(node_a1.action.precond_pos) - set(node_a2.action.effect_rem)) != len(node_a1.action.precond_pos):
            return True
        if len(set(node_a1.action.precond_neg) - set(node_a2.action.effect_add)) != len(node_a1.action.precond_neg):
            return True
        if len(set(node_a2.action.precond_pos) - set(node_a1.action.effect_rem)) != len(node_a2.action.precond_pos):
            return True
        if len(set(node_a2.action.precond_neg) - set(node_a1.action.effect_add)) != len(node_a2.action.precond_neg):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for precond_node_a1 in node_a1.parents:
            for precond_node_a2 in node_a2.parents:
                if precond_node_a1.is_mutex(precond_node_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for precond_node_s1 in node_s1.parents:
            for precond_node_s2 in node_s2.parents:
                if not precond_node_s1.is_mutex(precond_node_s2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            #print("goal: ", goal)
            for pos, level in enumerate(self.s_levels):
                level_symbols = [state.symbol for state in level]
                #print("pos: ", pos)
                #print("level_symbols: ", level_symbols)
                if goal in level_symbols:
                    goal_levels.add(pos + 1)
                    break
        return sum(goal_levels)
